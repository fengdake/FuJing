<template>
  <a-card :bordered="false">
          ...
          <template v-if="toggleSearchStatus">
            ...
            <a-col :md="6" :sm="8">
              <a-form-item label="所属单位">
                <a-select
                  showSearch
                  v-model="queryParam.ssdw"
                  placeholder="请选择所在单位">
                  <a-select-option
                    v-for="(item,index) in ssdwlist"
                    :key="index"
                    :value="item.text">
    ...
    <div class="table-operator">
      <a-button @click="handleAdd" v-has="'add:zj'" type="primary" icon="plus">新增</a-button>
      <a-button type="primary" icon="download" class="moBan" @click="handleExportXls('辅警人员基本信息')">导出</a-button>
      <a-dropdown v-if="selectedRowKeys.length > 0" v-has="'add:zj'">
        <a-menu slot="overlay">
    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-divider type="vertical"/>
          <a-popconfirm title="确定删除吗?" :disabled="disabledAuth('cz:fj')" @confirm="() => handleDelete(record.id)">
            <a>删除</a>
          </a-popconfirm>
    ...
    <!-- 表单区域 -->
    <fjJbxx-modal ref="modalForm" @Refresh="Refresh"></fjJbxx-modal>
    <chakanjbxx ref="chakan"></chakanjbxx>
  </a-card>
</template>

<script>
import FjJbxxModal from './modules/FjJbxxModal'
import chakanjbxx from './modules/chakanjbxx'
