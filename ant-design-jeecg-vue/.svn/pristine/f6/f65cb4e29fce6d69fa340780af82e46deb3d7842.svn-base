    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="默认跳转地址">
          <a-input placeholder="请输入路由参数 redirect" v-decorator="[ 'description',{}]"/>
        </a-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      loadTree(){
        var that = this;
        queryTreeList({sysName:Vue.ls.get("SYS_NAME")}).then((res)=>{
          if(res.success){
      ...
      add () {
        this.edit({status:'1',permsType:'1',route:true,description:Vue.ls.get("SYS_NAME")});
      },
      edit (record) {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            let sysName = Vue.ls.get("SYS_NAME");
            if (sysName == '' || 'common' == formData.description)
            {
              sysName = formData.description;
            }
            formData.description = sysName;
